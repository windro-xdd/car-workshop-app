...
export const ItemForm: React.FC<ItemFormProps> = ({ onSubmit, isLoading = false, initialData }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: type === 'checkbox' ? (e.target as HTMLInputElement).checked : name === 'unitPrice' ? parseFloat(value) || 0 : value,
    }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit(formData);
    if (!initialData) {
      setFormData({ code: '', name: '', category: '', unitPrice: 0, isActive: true });
      setErrors({});
    }
  };
            ...
            <>
              <div className="flex gap-2">
                <input
                  ...
                  value={formData.category}
                  onChange={handleChange}
                  placeholder="e.g., Tires, Batteries, Paint..."
                  className={`flex-1 px-3 py-2 text-sm bg-white border rounded-lg shadow-sm placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 transition-all duration-200 ${
                    errors.category ? 'border-red-300 focus:border-red-500 focus:ring-red-500/20' : 'border-zinc-200 focus:border-brand-500'
                  }`}
                  aria-invalid={!!errors.category}
                  aria-describedby={errors.category ? 'category-error' : undefined}
                  required
                ...
                <button
                  ...
                  className="px-3 py-2 text-sm font-medium text-zinc-700 bg-zinc-100 border border-zinc-300 rounded-lg hover:bg-zinc-200 transition-all duration-200"
                >
                  Cancel
                </button>
              </div>
        ...
        <div className="flex items-center mb-6">
          <label htmlFor="isActive" className="flex items-center cursor-pointer">
            <input
              ...
              onChange={handleChange}
              className="w-4 h-4 rounded border-zinc-300 text-brand-600 focus:ring-2 focus:ring-brand-500 focus:ring-offset-0 transition-all duration-200"
            />
            <span className="ml-2.5 text-sm font-medium text-zinc-700">In Stock</span>
          </label>
          <span className={`ml-3 text-xs px-2.5 py-1 rounded-full font-medium ${
            ...
            {formData.isActive ? 'Available' : 'Out of Stock'}
          </span>
        </div>
